...
if [ "$2" = "-file" ] || [ "$2" = "-everything" ]; then 
  ...
  for URL1 in $(cat $1); do
    ...
    # Запускаем первую команду
    katana -u "$URL1" -d 5 -crawl-scope $1 -js-crawl -jsluice -crawl-duration 15m -known-files all -disable-redirects -headless -c 150 -p 1 -silent -f ufile >> "files/${URL2}_active.txt"
    echo "Active DONE"
    
    # Запускаем вторую команду
    katana -u "$URL1" -d 5 -crawl-scope $1 -js-crawl -jsluice -crawl-duration 15m -known-files all -disable-redirects -c 150 -p 1 -ps -silent -f ufile >> "files/${URL2}_passive.txt"
    echo "Passive DONE"
...
if [ "$2" = "-path" ] || [ "$2" = "-everything" ]; then
  ...
  for URL1 in $(cat $1); do
    ...
    # Запускаем первую команду
    katana -u "$URL1" -d 5 -crawl-scope $1 -js-crawl -jsluice -crawl-duration 15m -known-files all -disable-redirects -headless -c 150 -p 1 -silent -f udir >> "paths/${URL2}_active.txt"
    echo "Active DONE"
    
    # Запускаем вторую команду
    katana -u "$URL1" -d 5 -crawl-scope $1 -js-crawl -jsluice -crawl-duration 15m -known-files all -disable-redirects -c 150 -p 1 -ps -silent -f udir >> "paths/${URL2}_passive.txt"
    echo "Passive DONE"
